...
class PaiementRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'montant' => 'required|numeric|min:0|max:10000000',
            'datePaiement' => 'required|date',
            'statutPaiement' => 'required|in:PAYEE,NON_PAYEE,REMBOUSE',
            'telephone' => 'nullable|string|max:20|regex:/^(\+221)?[0-9\s\-\(\)]{8,}$/',
            'operateur' => 'nullable|in:ORANGE_MONEY,FREE_MONEY,WAVE',
        ...
        if ($this->isMethod('PUT') || $this->isMethod('PATCH')) {
            ...
            $rules['modePaiement'] = 'sometimes|in:EN_LIGNE,EN_ESPECE';
            $rules['montant_paye'] = 'sometimes|numeric|min:0|max:10000000';
            $rules['datePaiement'] = 'sometimes|date';
    ...
    public function messages(): array
    {
        return [
            ...
            'modePaiement.in' => 'Le mode de paiement doit être "EN_LIGNE" ou "EN_ESPECE"',
            'montant.required' => 'Le montant payé est obligatoire',
            'montant.numeric' => 'Le montant payé doit être un nombre',
            'montant.min' => 'Le montant payé ne peut pas être négatif',
            'montant.max' => 'Le montant payé ne peut pas dépasser 10,000,000 FCFA',
            'datePaiement.required' => 'La date de paiement est obligatoire',
            'statutPaiement.required' => 'Le statut du paiement est obligatoire',
            'statutPaiement.in' => 'Le statut du paiement n\'est pas valide',
    ...
    {
        $this->merge([
            'telephone' => $this->numero_telephone ? trim($this->telephone) : null,
            'reference' => $this->reference_transaction ? trim($this->reference_transaction) : null
        ]);
    }
